...
export async function listTasks(req, res, next) {
  try {
    ...
    const tasks = await taskService.getAllTasks(req.user.id, { completed });
    res.json({ success: true, data: tasks.map(withDeadlineInfo) });
  } catch (e) { next(e); }
}
export async function getTask(req, res, next) {
  try { requireId(req.params.id); res.json({ success: true, data: withDeadlineInfo(await taskService.getTaskById(req.params.id, req.user.id)) }); }
  catch (e) { next(e); }
}
export async function createTask(req, res, next) {
  try {
    ...
    const task = await taskService.createTask(result.data, req.user.id);
...
export async function updateTask(req, res, next) {
  try {
    ...
    if (!result.valid) throw new AppError(result.errors.join(" "), 400);
    res.json({ success: true, data: withDeadlineInfo(await taskService.updateTask(req.params.id, result.data, req.user.id)) });
  } catch (e) { next(e); }
}
export async function deleteTask(req, res, next) {
  try { requireId(req.params.id); await taskService.deleteTask(req.params.id, req.user.id); res.json({ success: true, data: { id: req.params.id } }); }
  catch (e) { next(e); }
}
export async function startTask(req, res, next) {
  try { requireId(req.params.id); res.json({ success: true, data: withDeadlineInfo(await taskService.startTask(req.params.id, req.user.id)) }); }
  catch (e) { next(e); }
}
export async function completeTask(req, res, next) {
  try { requireId(req.params.id); res.json({ success: true, data: withDeadlineInfo(await taskService.completeTask(req.params.id, req.user.id)) }); }
  catch (e) { next(e); }
}
